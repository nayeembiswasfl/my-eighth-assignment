...
const REGISTERED_USER_KEY = "tilecraft_registered_user";
...
export function registerDummyUser({ email, name, image } = {}) {
  const user = {
    name: name || email?.split("@")[0] || defaultUser.name,
    email: email || defaultUser.email,
    image: image || ""
  ...
  window.localStorage.setItem(REGISTERED_USER_KEY, JSON.stringify(user));
  return user;
}
